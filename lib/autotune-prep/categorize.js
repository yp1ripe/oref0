    ...
    for (i=bucketedData.length-5; i >= 0; --i) {
        ...
        if (typeof(bucketedData[i].glucose) !== 'undefined' && typeof(bucketedData[i+4].glucose) !== 'undefined') {
            ...
            if( CRCarbs > 0  ) {
                ...
                if( BG < CRMinBG.val  || CRMinBG.val == null ) {
                    CRMinBG.val = BG;
                    CRMinBG.time = BGDate;
                }
            }
        ...
        if (mealCOB > 0 && absorbing || calculatingCR ) {
            ...
            if ( iob.iob < currentBasal/2 && mealCOB == 0 && absorbing && (end_meal_avgdev >= 0 || BGTime-CRInitialCarbTime>=3600*1000*CRCarbs/10)) {
                absorbing = 0;
            ...
            if (basalBGI > -4 * BGI) {
                ...
                //} else  {
                    if( type != "ISF" ) {
                        ...
                        intervalDataISF = {
                            startTime: new Date(glucoseDatum.date),
                            dosed: 0,
                            startBG: BG,
                            startIOB: iob
                        };
