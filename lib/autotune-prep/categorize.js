...
function categorizeBGDatums(opts) {
    ...
    for (i=bucketedData.length-5; i >= 0; --i) {
        ...
        if(debug_wizard && wizardInsulinIOB != null &&  wizardInsulinPending > 0 ) {
            console.error("iob.bolusiob", iob.bolusiob);
            console.error("wizardInsulinIOB.bolusiob", wizardInsulinIOB.bolusiob);
            console.error("rounded dose",Math.round((iob.bolusiob-wizardInsulinIOB.bolusiob) * 10 )/10);
            console.error("expected",Math.round((wizardInsulinPending-0.1)*10)/10  );
        }
        if( wizardInsulinPending > 0  && 
            Math.round((iob.bolusiob-wizardInsulinIOB.bolusiob) * 10 )/10 >= Math.round((wizardInsulinPending-0.1)*10)/10 
        ) {
        ...
        if (mealCOB > 0 && absorbing || calculatingCR ) {
            ...
            if (!calculatingCR) {
                if( prevGlucoseDatum.mealAbsorption != "end" && prevIob != null && (iob in prevIob) && prevIob.iob >= iob.iob ) {
...
//                if ( CRElapsedMinutes < 60 || ( i===1 && mealCOB > 0 ) || !CRDatum.CRInitialIOB ) {
    ...
    function tooCloseToMeal( el , index, arr ) { 
        var pass=true; 
        //console.error( "deciding on",(arr === UAMGlucoseData ? 'uam' : 'bas' ), new Date( el.date ) );
        CRData.forEach( function(CRDatum) {
            //console.error("CRDatum.CRInitialCarbTime ",CRDatum.CRInitialCarbTime);
            //console.error("CRDatum.CREndTime ",CRDatum.CREndTime);
            //console.error("alternative end", ( "alternative" in CRDatum ) ? CRDatum.alternative.endTime: CRDatum.CREndTime );
            //console.error("filterout end", new Date( CRDatum.CRInitialCarbTime.getTime() + 5*3600*1000 ) );
            var dt = new Date( Math.max(
                   CRDatum.CREndTime,
                   //( "alternative" in CRDatum ) ? CRDatum.alternative.endTime: CRDatum.CREndTime ,
                   new Date( CRDatum.CRInitialCarbTime.getTime() + 5.5*3600*1000 )
               ) );
            //console.error("dt ", dt );
            if(el.date >= CRDatum.CRInitialCarbTime && CRDatum.CREndTime != null && 
               el.date <=  dt
            )  {
                pass=false;
            }
        });
        if(debug_categ)console.error( "deciding on",(arr === UAMGlucoseData ? 'uam' : 'bas' ), new Date( el.date ) , pass ? 'passed' : 'not passed');
        return pass; 
    }
    ...
    console.error("Uams before:",UAMLength);
    UAMGlucoseData = UAMGlucoseData.filter( tooCloseToMeal );
    UAMLength = UAMGlucoseData.length;
    console.error("Uams after meals filter:",UAMLength);
    basalGlucoseData = basalGlucoseData.filter( between );
    var basalLength = basalGlucoseData.length;
    console.error("Basals before:",basalLength);
    basalGlucoseData = basalGlucoseData.filter( tooCloseToMeal );
    basalLength = basalGlucoseData.length;
    console.error("Basals after meals filter:",basalLength);
