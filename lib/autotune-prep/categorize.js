...
function categorizeBGDatums(opts) {
    ...
    if( opts.end_meal_if_avgdev_le != 0.0) {
    ...
    for (i=bucketedData.length-5; i > 0; --i) {
        ...
        if (treatment) {
            ...
            if ( treatmentTime < BGTime ) {
                if (treatment.carbs >= 1) {
                    ...
                    if ( treatment.carbs <= 15 && calculatingCR 
                       //     && glucoseDatum.glucose < 90.90 
                        ) { 
                        // most likely fast carbs
                        tempImpact = Math.max( profile.min_5m_carbimpact, tempImpact);
                        tempImpact += 15;
        ...
        if ( ( absorbing || mealCOB > 0 ) && deviation > 2*basalBGI && deviation > profile.min_5m_carbimpact 
            || (i<=savedI-4) ) {
            absorbing = 1;
        }
        ...
        if ( mealCOB >= 0 && absorbing ) {
            ...
            if( impactDecay.length && mealCOB <= impactDecay[impactDecay.length-1] ) {
                tempImpact -= 15;
        ...
        console.error(absorbing.toString(),"mealCOB:",mealCOB.toFixed(1),"mealCarbs:",mealCarbs.toFixed(1),"BGI:",BGI.toFixed(1),"IOB:",iob.iob.toFixed(2),"at",BGTime,"dev:",dev5m,"ci",ci,"avgDev:",deviation,"avgDelta:",avgDelta,type,BG,myCarbs);
    }
