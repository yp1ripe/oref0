...
//                if ( CRElapsedMinutes < 60 || ( i===1 && mealCOB > 0 ) || !CRDatum.CRInitialIOB ) {
        ...
        if (  absorbing && ( mealCOB > 0 ||  mealCarbs > 0) ) {
          ...
          if ( type === "csf" ) {
            ...
            function splitCombined(glucoseDatum ) {
                var R1 = Math.abs(iob.basalActivity)/(Math.abs(iob.bolusActivity)+Math.abs(iob.basalActivity));
                if(debug_categ) {
                    console.error("!!!iob.activity "+iob.activity +" iob.bolusActivity "+iob.bolusActivity
                        +" iob.basalActivity "+iob.basalActivity
                    );
                }
                var R2 = 1-R1;
                ...
                //if( R1 > 0 ) { // 7/24
                    if(debug_categ) { console.error("unusual basal BGI:"+bas.BGI+" dev "+bas.deviation+" R "+R1); }
                    type="basal";
                ...
                if(R2>= BOLUS_ACTIVITY_FRACTION_THRESHOLD && avgDelta < 0 && isf.deviation <= 6 && iob.bolusActivity>= 0.00015) {
                //if( R2> 0  && isf.deviation <=6 ) {
        ...
        if( type.substring(0,3) == "ISF" ) { //: was && intervalDataISF != null
        /*    var tmp_treatments = find_insulin(IOBInputs);
            var dosedOpts = {
                treatments: tmp_treatments
                , profile: opts.profile
        ...
        console.error(
            ...
            "σ"+dev5m.padStart(6,' ')+" avσ"+deviation.padStart(6,' ')+(absorbing && ci != deviation? '>'+ci : '').padEnd(3,' ')+
