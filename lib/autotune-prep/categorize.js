...
function categorizeBGDatums(opts) {
    ...
    for (i=bucketedData.length-5; i >= 0; --i) {
	    ...
	    if( !PISA && (Gv <= GinPISA)   ) {
            ...
            console.error("Pre-PISA Gv ", Gv, "r1PISA",  r1PISA, "sdg",sdg," prevSdg",prevSdg);
            if( r1PISA > Gratio  || prevGv >= 0 || ( ( sdg > 0 ) && ( prevSdg <= 0 ))) {
        ...
        if(PISA && Gv > GoutPISA) {
            if( (glucoseDatum.smoothed < glucoseDatum.glucose) && ( glucoseDatum.glucose - glucoseDatum.smoothed > Math.abs(GinPISA)*5 )) {
            ...
            if( PISA && (iPISA-i>=3) && ( sdg < 0 ) && ( prevSdg >= 0 ) ) {
        ...
        if ( mealCOB >= 0 && absorbing ) {
            ...
            if(CRInitialCarbTime != null  && BGTime-CRInitialCarbTime >= 5*3600*1000 &&
                BGTime-CRInitialCarbTime <= 5*3600*1000+900*1000 ) {
        ...
        if (mealCOB > 0 && absorbing || calculatingCR ) {
            ...
            if ( mealCOB == 0 && absorbing  && impactDecay.length == 0
                 && ( (iob.iob < currentBasal/2) && (Math.abs(iob.basaliob) < currentBasal/2) || (BGTime-CRInitialCarbTime>=3600*1000*CRCarbs/10) && limit_decay_time)) {
                absorbing = 0;
        ...
        if (  absorbing && ( mealCOB > 0 ||  mealCarbs > 0) ) {
          ...
          }
          {
            var splitCombined = function (glucoseDatum ) {
                ...
                //var sum_abs = Math.abs(iob.bolusActivity)+Math.abs(iob.basalActivity)
                ...
                if( basBGI > 0 ) {
                    ...
                    bas.deviation = avgDelta*R1 - BGI*R1 ;
                    bas.BGI = BGI*R1;
                }
                var res = 0;
                //if((R1>=1-BOLUS_ACTIVITY_FRACTION_THRESHOLD && type == "uam") || avgDelta > 0 || basBGI < 0  ) { // 7/24
                //if( bas.deviation <= 6 && R1>=(1-BOLUS_ACTIVITY_FRACTION_THRESHOLD)/4  ) { // 7/24
                if(debug_categ)console.error("   bas.deviation "+bas.deviation+" basBGI "+bas.BGI);
                if( bas.deviation <= 6 && (-basalActivity*4 < -ztDIABasalAct ) && ( Math.abs(basalActivity) > corr)) { // 7/24
                //if( R1 > 0 ) { // 7/24
                ...
                //isf.deviation = avgDelta*R2 - (BGI-basBGI);
                // BGI = ( bolusBGI + basBGI ), so (BGI - basBGI ) == bolusBGI+basBGI - basBGI
                if( basBGI > 0 ) {
                    isf.deviation = avgDelta -  corrBolBGI;
                    isf.BGI =  corrBolBGI;
                } else {
                    isf.deviation = avgDelta*R2 - BGI*R2 ;
                    isf.BGI = BGI*R2;
                }
                var a_max = profileData.autosens_max+0.1;
                var a_min = profileData.autosens_min-0.1;
                if ( (avgDelta <= 0 || Math.abs(corrActivity) >= Math.abs(ztDIABasalAct)/4 )&& lag <= 10*60*1000) {
                ...
                if(debug_categ) { console.error("   supposed isf val "+Math.round(isf_val * 100 )/100+'('+Math.round(isf_val*.0555*100)/100 + ") dev "+Math.round(isf.deviation *10000)/10000+"BGI"+Math.round(isf.BGI *100)/100+" R "+Math.round(R2*10000)/10000+" > "+ (Math.abs(corrActivity) >= Math.abs(ztDIABasalAct)/4) + " Dumb isf " + Math.round( dumb_isf *100)/100+" isf4 "+Math.round( dumb4_isf *100)/100);
                }
                if(debug_categ && !(isf_val <= sens / a_min && isf_val >= sens / a_max )) console.error("   isf is out of bounds ",isf_val, sens/a_max, sens/a_min );
                if(//(avgDelta < 0 || avgDelta == 0 && (type == "ISF" || type == "uam") )&& 
                    isf.deviation <= 6 && corrActivity >= Math.abs(ztDIABasalAct)/4 && ((corr >0  && basBGI >0) || (basBGI <= 0))
                    //&& R2> BOLUS_ACTIVITY_FRACTION_THRESHOLD
                    //&& !(isf_val <= sens/ 2 || isf_val >= sens*2)
                    && isf_val > 0
                ) {
