...
function categorizeBGDatums(opts) {
    ...
    for (i=bucketedData.length-5; i >= 0; --i) {
            ...
            if (treatment) {
                ...
                if ( treatmentTime <= BGTime ) {
                    if( treatment != null && treatment.hasOwnProperty("bolusCalc") == true  ) {
                        ...
                        console.error( "Bolus Wizard entry at ", treatmentDate.toString().split(' ')[4], " wizardCarbsPending =",
                    ...
                    if (treatment.carbs >= 1 && treatment.hasOwnProperty("duration")==false) {
                        ...
                        console.error("treatment.carbs=",treatment.carbs,"wizardCarbsPending",wizardCarbsPending);
                        if( treatment.carbs == wizardCarbsPending ) {
                            wizardCarbsPending = 0;
                            console.error("Pending carbs found",treatment.carbs);
                        }
                    }
        ...
        if( wizardInsulinPending > 0  && Math.round((iob.bolusiob-wizardInsulinIOB.bolusiob) * 10 )/10 >= (wizardInsulinPending-0.1) ) {
            ...
            console.error("Pending insulin found: ", iob.bolusiob);
        ...
        if ( mealCOB >= 0 && absorbing ) {
            ...
            if(CRInitialCarbTime != null  && BGTime-CRInitialCarbTime >= 4*3600*1000 &&
...
//                if ( CRElapsedMinutes < 60 || ( i===1 && mealCOB > 0 ) || !CRDatum.CRInitialIOB ) {
        ...
        if (mealCOB > 0 || absorbing || mealCarbs > 0) {
          ...
          if ((iob.iob > 2 * currentBasal || deviation > 6 || uam) && (wizardCarbsPending == 0 || wizardCarbsPending >0 && type != "ISF") ) {
            ...
            if ( type !== "uam" ) {
                ...
                //console.error(glucoseDatum.uamAbsorption,"unannnounced meal absorption iob ",iob.iob," deviation ", deviation, " uam ", uam, " cb " , currentBasal);
                console.error(glucoseDatum.uamAbsorption,"unannnounced meal absorption");
            }
            type="uam";
            ...
            if ((basalBGI > -4 * BGI ) && ( BGI <= 0 )  || BGI > 0 || iob.bolusiob == 0) {
                ...
                /// else \leftBrace
                    ...
                    type="ISF";
                    var R1 = Math.abs(iob.basalActivity)/(Math.abs(iob.activity)+Math.abs(iob.basalActivity));
                    var R2 = 1-R1;
                    var debug = opts.dbg_output != null && opts.dbg_output & 4 ? true : false;
                    if(R2 > 0 && opts.combined_isf_basal) {
                        ...
                        if(debug) { console.error("unusual basal BGI:"+bas.BGI+" dev "+bas.deviation+" R "+R1); }
                        ...
                        if(debug) { console.error("unusual isf BGI:"+isf.BGI+" dev "+isf.deviation+" R "+R2); }
                        ISFGlucoseData.push(isf);
                    } else {
                        if(debug) {console.error("usual");}
		...
		for (ratI=0; ratI < ISFGlucoseData.length; ++ratI) {
			    ...
			    //console.error("skipping ISF dated "+ new Date( ISFGlucoseData[ratI].date) );
			    continue;
	            }
		...
		CR = Math.round( percentile( ratios, 0.50 )  * sens  *.0555 * 100 )/100;
        }
        console.error(
    ...
    CRData.forEach(function(CRDatum) {
        ...
        //console.error("meals loop iobinputs clock",IOBInputs.clock);
