...
function categorizeBGDatums(opts) {
    ...
    for (i=bucketedData.length-5; i >= 0; --i) {
        ...
        var BGstr = (BG.toString() + "|" + BGmmol.toFixed(1)).padStart(8,' ');
 
        console.error(
            BGTime+' '+
            BGstr+" ∆"+delta.toString().padStart(3,' ')+" av∆ "+avgDelta.padStart(5,' ')+
            " BGI "+BGI.toFixed(1).padStart(4,' ')+"|"+BGImmol,
            "σ "+dev5m.padStart(5,' ')+" avσ "+deviation.padStart(5,' ')+( absorbing? ( ci != deviation? '->'+ci : '   ') : '   ')+
