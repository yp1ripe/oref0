...
//                if ( CRElapsedMinutes < 60 || ( i===1 && mealCOB > 0 ) || !CRDatum.CRInitialIOB ) {
    ...
    function tooCloseToMeal( el ) { 
        var pass=true; 
        //console.error( "deciding on", new Date( el.date ) );
        CRData.forEach( function(CRDatum) {
            ...
            if(el.date >= CRDatum.CRInitialCarbTime && CRDatum.CREndTime != null && 
               el.date <=  dt
                ...
                pass=false;
                //console.error("not passed the filter");
            }
        });
        //if(pass) console.error("passed the filter");
        return pass; 
    }
    ...
    basalLength = basalGlucoseData.filter( tooCloseToMeal );
    var basalLength = basalGlucoseData.length;
    console.error("Basals after:",basalLength);
