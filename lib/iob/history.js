...
function splitTimespanWithOneSplitter(event,splitter) {
    ...
    if (splitter.type === 'recurring') {
        ...
        if (event.duration > 30 && split30 || (startMinutes < splitter.minutes && endMinutes > splitter.minutes) || (endMinutes > 1440 && splitter.minutes < (endMinutes - 1440))) {
...
//    let caller = whoIsMyDaddy();
    ...
    for (i=0; i < splitHistory.length; i++) {
        ...
        if (currentItem.duration > 0) {
            ...
            //process.stderr.write("currentDate: "+new Date (currentItem.date).toLocaleString()+" rate:"+netBasalRate+" ");
            //process.stderr.write("duration: "+currentItem.duration+" netBasalAmount "+netBasalAmount+"\n");
            var grouping = ''
            if ( debug ) {
                grouping = 
		    ...
		    minute: '2-digit'
                ...
                " rate:"+netBasalRate.toFixed(2).padStart(5,' ')+" "+"duration: "+Math.round(currentItem.duration).toString().padStart(2,' ');
            }
