...
function splitTimespanWithOneSplitter(event,splitter) {
    ...
    if (splitter.type === 'recurring') {
        ...
        //if(endMinutes > 1440 ) console.error("split-split",splitter,event);

        // 1440 = one day; no clean way to check if the event overlaps midnight
        // so checking if end of event in minutes is past midnight

        if (event.duration > 30 && split30 || (startMinutes < splitter.minutes && endMinutes > splitter.minutes) || (endMinutes > 1440 && splitter.minutes < (endMinutes - 1440)) || (splitter.minutes == 0 && endMinutes > 1440)) {
...
*/
            if (endMinutes > 1440 && splitter.minutes < (endMinutes - 1440) && splitter.minutes == 0  ) {
                splitPoint = 1440;
                event1Duration = splitPoint - startMinutes;
            }
	    if( (event1Duration >= 30 || (event.duration - event1Duration) >= 30 || event1Duration == 0) &&
                 event.duration >30 && split30 )  {
                event1Duration = 30;
            } else {
                event1Duration = splitPoint - startMinutes;
	    }
            ...
            var event1EndDate = moment(event.started_at).add(event1Duration,'minutes').toDate();

            event1.duration = event1Duration;

            event2.duration  = event.duration - event1Duration;
            event2.timestamp = event1EndDate;
            event2.started_at = new Date(event2.timestamp);
            event2.date = event2.started_at.getTime();

            resultArray = [event1,event2];
            //if(endMinutes > 1440 ) console.error("resultArray ",resultArray,"splitPoint", splitPoint);
        }
    }

    //if(resultArray.length>1)console.error("resultArray ",resultArray,"splitter", splitter);
    return resultArray;
}
...
function splitTimespan(event, splitterMoments) {
    ...
    while(splitFound) {
        ...
        _.forEach(results,function split1(o) {
            splitFound = false;
            _.forEach(splitterMoments,function split(p) {
                ...
                if (splitResult.length > 1) {
                    ...
                    splitFound = true;
	            //if(splitFound) console.error("resultArray innnermost-loop found",o,p,resultArray);
                    return false;
                //} else {
                    // console.error("resultArray innermost-loop not found",o,p,resultArray);
                }
            });

            if (!splitFound) {
		resultArray = resultArray.concat([o]);
                    //if(splitFound) console.error("resultArray inner-loop not-found",o,resultArray);
	    }
            return true;
        });
        //console.error("resultArray outermost-loop",resultArray);
        results = resultArray;
        //console.error("results out-loop",results);
    }
...
//    let caller = whoIsMyDaddy();
    ...
    split30 = inputs.split30 !== null ? inputs.split30 : true;
    ...
    for (var i=0; i < pumpHistory.length; i++) {
        ...
        //console.error(new Date( current.created_at.getTime()).to_string().split(" ")[4]);
    ...
    for (i=0; i < pumpHistory.length; i++) {
        ...
        //if( !("pumpType"  in current))console.error(current);
        ...
        var max_dia_ago = new Date( now.getTime() - DIA*60*60*1000 );
        var durationInMilliseconds =0;
        if ( "durationInMilliseconds" in current ) {
	  if( current.durationInMilliseconds > 0 ) {
            durationInMilliseconds = current.durationInMilliseconds;
	  }
        }
        if (currentRecordTime > lastRecordTime )  {
            ...
            continue;
        } else if ( currentRecordTime.getTime()+durationInMilliseconds  <= max_dia_ago) {
            //console.error("Skipped ",new Date(currentRecordTime).toLocaleString(),
            //                " dur ",durationInMilliseconds,
            //                " end ", new Date(currentRecordTime.getTime()+durationInMilliseconds).toLocaleString(),
            //                " <= ",max_dia_ago.toLocaleString());
            continue;
        } else {
        ...
        if (current._type === "Bolus") {
            ...
            tempBoluses.push(temp);
        } else if (current.enteredBy === "xdrip" || current.enteredBy === "sync") {
            var temp = {};
            temp.timestamp = current.timestamp;
            temp.started_at = new Date(tz(temp.timestamp));
            temp.date = temp.started_at.getTime();
            ...
            temp.insulin = current.insulin;
            tempBoluses.push(temp);
        } else if (current.eventType === "OpenAPS Offline" && current.enteredBy === "openaps://AndroidAPS") {
            var temp = {};
            temp.rate = 0;
            temp.duration = current.durationInMilliseconds/1000/60;
            temp.timestamp = current.created_at;
            temp.started_at = new Date(tz(temp.timestamp));
            temp.date = temp.started_at.getTime();
            ...
            temp.date = temp.started_at.getTime();
            tempHistory.push(temp);
            //if( !("pumpType" in current))console.error(current,temp);
    ...
    //if( debug ) console.error(tempHistory);
    ...
    //if(debug) console.error("splitHistory0", splitHistoryByBasal);
    ...
    if (suspend_zeros_iob) {
        ...
        // resume
        var max_dia_ago = now.getTime() - DIA*60*60*1000;
        var firstResumeStarted = new Date(firstResumeTime);
        var firstResumeDate = firstResumeStarted.getTime()
    ...
    //if(debug) console.error("splitHistory ", splitHistory);
    ...
    for (i=0; i < splitHistory.length; i++) {
        ...
        if (currentItem.duration > 0) {
            ...
            if ( profile.exercise_mode && profile.temptargetSet && target_bg >= normalTarget + 5 ) {
                ...
                process.stderr.write("currentDate: "+new Date (currentItem.date).toString()+" Autosens ratio: "+sensitivityRatio+"; ");
            }
            if ( sensitivityRatio ) {
                currentRate = currentRate * sensitivityRatio;
            }
            var roundedCurrentRate = Math.round(currentRate*20)/20;
            var netBasalRate = currentItem.rate - roundedCurrentRate;
            var netBasalRate = Math.round(netBasalRate*20)/20;
            //process.stderr.write("currentDate: "+new Date (currentItem.date).toLocaleString()+" ");
            //process.stderr.write("netBasalRate: "+netBasalRate+" roundedCurrentRate "+roundedCurrentRate+" ");
            //process.stderr.write("rate: "+currentItem.rate);
            //if (netBasalRate < 0) { tempBolusSize = -0.05; }
            //else { tempBolusSize = 0.05; }
            if (netBasalRate < 0) { tempBolusSize = -0.00083333; }
            else { tempBolusSize = 0.00083333; }
            //var netBasalAmount = Math.round(netBasalRate*currentItem.duration/60*1000)/1000
            var netBasalAmount = Math.round(netBasalRate*currentItem.duration/60*10000)/10000
            //if(debug)process.stderr.write("currentDate: "+new Date (currentItem.date).toLocaleString()+" rate:"+netBasalRate+" ");
            //if(debug)process.stderr.write("duration: "+currentItem.duration+" netBasalAmount "+netBasalAmount+"\n");
            var grouping = ''
            if ( debug ) {
                grouping = 
                new Date (currentItem.date).toLocaleString("en-US", {
			year: '2-digit',
			month: '2-digit',
			day: '2-digit'
		})+" "+
                new Date (currentItem.date).toLocaleString("en-GB", {
		        hourCycle: 'h24',
		        hour: '2-digit',
		        minute: '2-digit'
		})+"-"+
                new Date (currentItem.date+Math.round(currentItem.duration)*60*1000).toLocaleString("en-GB", {
		    hourCycle: 'h24',
		    hour: '2-digit',
		    minute: '2-digit'
		})+
                " rate:"+netBasalRate.toFixed(2).padStart(5,' ')+" "+"duration: "+Math.round(currentItem.duration).toString().padStart(3,' ');
            }
            var tempBolusCount = Math.round(netBasalAmount / tempBolusSize);
            var tempBolusSpacing = currentItem.duration / tempBolusCount;
            //console.error("Count = ",tempBolusCount," Size = ", tempBolusSize," Spacing = ", tempBolusSpacing);
            var netTotal = netTotal + netBasalAmount;
            for (j=0; j < tempBolusCount; j++) {
                ...
                if( debug ) {
                     tempBolus.grouping = grouping;
		}
            }
        }
    }
    //console.error("netTotal",netTotal);
