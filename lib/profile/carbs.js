...
function crLookup(profile, timestamp) {
    ...
    var cr_data = profile.carb_ratios.schedule;
    cr_data = _.sortBy(cr_data, function(o) { return o.offset; });

    var crSchedule = cr_data[cr_data.length - 1];

    if (cr_data[0].offset !== 0) {
        return  null;
    }

    var endMinutes = 1440;

    for (var i = 0; i < cr_data.length - 1; i++) {
        var currentCR = cr_data[i];
        var nextCR = cr_data[i+1];
        if (nowMinutes >= currentCR.offset && nowMinutes < nextCR.offset) {
            endMinutes = nextCR.offset;
            crSchedule = cr_data[i];
            break;
        }
    }

    return crSchedule.ratio;
}
carbRatioLookup.carbRatioLookup = carbRatioLookup;
carbRatioLookup.crLookup = crLookup;
