...
function tuneAllTheThings (inputs) {
    ...
    CRData.forEach(function(CRDatum) {
        ...
        endTime = endArr[4];
        if( CRInitialCarbTime < dayStartTime && (inputs.params["strict-24h-meals"] == true ) ) return;
        if( CRInitialCarbTime > dayEndTime && (inputs.params["strict-24h-meals"] == true )) return;
        console.error(
    ...
    if (pumpBasalProfile && pumpBasalProfile[0]) {
        for (hour=0; hour < 24; hour++) {
            ...
            if( Math.round(newHourlyBasalProfile[hour].rate*lRoundTo)/lRoundTo != Math.round(hourlyPumpProfile[hour].rate*lRoundTo)/lRoundTo  ||  (hour in hoursTuned) && (newHourlyBasalProfile[hour].rate != hourlyBasalProfile[hour].rate)) {
