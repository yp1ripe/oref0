...
function tuneAllTheThings (inputs) {
    ...
    CRData.forEach(function(CRDatum) {
        ...
        var CRInsulinReq = CRBGChange / currISF;
        //var CRIOBChange = CRDatum.CREndIOB - CRDatum.CRInitialIOB;
        var CRIOBChange = CRDatum.CRInitialIOB.iob - CRDatum.CREndIOB.iob;
        ...
        //console.error(CRBGChange, CRInsulinReq, CRDatum.CRInsulin,CRIOBChange, CRDatum.CRInsulinTotal);
        var CRInitialCarbTime = new Date( CRDatum.CRInitialCarbTime ).getTime();
        var CREndTime = new Date(  CRDatum.CREndTime ).getTime();

        var bgnArr = new Date( CRDatum.CRInitialCarbTime ).toString().split(" ");
        bgnTime = bgnArr[4];
        ...
        var superBolusFactor = (CRDatum.CRInsulinTotal + CRDatum.superBolusSurplus) / CRDatum.CRInsulinTotal;
        ...
        if (CRDatum.CRInsulinTotal > 0) {
            ...
            scaledInsulin += CRDatum.CRInsulinTotal*saferScaleFactor;
        ...
        console.error(
            "    scaleFactor(limited): ",Math.round( saferScaleFactor *1000)/1000
        );
        console.error(
            "    superBolusFactor: ",Math.round( superBolusFactor *1000)/1000
        );
        console.error(
            "    scaledCR (g/u):",Math.round( CRDatum.CRCarbs / CRDatum.CRInsulinTotal / saferScaleFactor * 1000 )/1000
        );
    ...
    if (pumpBasalProfile && pumpBasalProfile[0]) {
        for (hour=0; hour < 24; hour++) {
            ...
            var minRate = Math.min(hourlyPumpProfile[hour].rate * autotuneMin,hourlyPumpProfile[hour].rate-1/roundTo);
    ...
    scaledCR = Math.max( totalCR*.75,scaledCR ) ;
