...
function tuneAllTheThings (inputs) {
    ...
    for (hour=0; hour < 24; hour++) {
        ...
        if( tmpRate/hourlyBasalProfile[hour].rate > autotuneMax ) {
            console.error("Hour "+hour+" rate "+newHourlyBasalProfile[hour].rate+" rounded "+tmpRate+" is still too high");
            tmpRate = Math.floor( newHourlyBasalProfile[hour].rate*roundTo )/roundTo;
            if( tmpRate/hourlyBasalProfile[hour].rate > autotuneMax )
                tmpRate -= 1/roundTo;
            console.error("further limiting to "+tmpRate);
        }
        if(tmpRate/hourlyBasalProfile[hour].rate < autotuneMin ) {
            console.error("Hour "+hour+" rate "+newHourlyBasalProfile[hour].rate+" rounded "+tmpRate+" is still too low");
            tmpRate = Math.ceil( newHourlyBasalProfile[hour].rate*roundTo )/roundTo;
            if( tmpRate/hourlyBasalProfile[hour].rate < autotuneMin )
                tmpRate += 1/roundTo;
            console.error("further limiting to "+tmpRate);
        }
