...
const actFractionsStrings = [ '5/32', '11/32', '1/4', '5/32', '1/16', '1/32' ];
...
function tuneAllTheThings (inputs) {
    ...
    for (var hour=0; hour < 24; hour++) {
        ...
        function applyHourBasals(hour,offset) {
            var offsetHour = hour + offset;
            if (offsetHour < 0) { offsetHour += 24; }
            if( inputs.params["dbg-output"] & 8 ) console.error("Hour", offsetHour," is getting", actFractionsStrings[Math.abs(offset)] ,"* 1/2 "," of total amount of ",basalNeeded,"adjustment");
            savedRate =  newHourlyBasalProfile[offsetHour].rate;
            var newHourlyBasal = (newHourlyBasalProfile[offsetHour].rate+basalNeeded * actFractions[ Math.abs(offset)]* .5);
                if( newHourlyBasal <= 0 ) {
                    if ( roundTo )
        ...
        if (basalNeeded > 0 ) {
            ...
            for (var offset=-5; offset <= 0; offset++) {
                ...
                applyHourBasals(hour-1,offset);
                applyHourBasals(hour,offset);
            }
