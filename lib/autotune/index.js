...
function tuneAllTheThings (inputs) {
    ...
    for (var hour=0; hour < 24; hour++) {
        ...
        if (basalNeeded > 0 ) {
            ...
            for (var offset=-5; offset <= 0; offset++) {
                ...
                if( inputs.params["dbg-output"] & 8 ) console.error("Hour", offsetHourPrev,"old rate", savedRatePrev ," new rate",newHourlyBasalProfile[offsetHourPrev].rate);
            }
    ...
    if (pumpBasalProfile && pumpBasalProfile[0]) {
        for (hour=0; hour < 24; hour++) {
            ...
            if( Math.round(newHourlyBasalProfile[hour].rate*lRoundTo)/lRoundTo != Math.round(hourlyPumpProfile[hour].rate*lRoundTo)/lRoundTo ) {
