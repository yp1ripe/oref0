...
const actFractions = [ 5/32, 11/32, 1/4, 5/32, 1/16, 1/32 ];
...
function tuneAllTheThings (inputs) {
    ...
    var dayStartTime = new Date( preppedGlucose.startDate ).getTime();
    var dayEndTime = new Date( preppedGlucose.endDate ).getTime();
    //console.error(preppedGlucose.startDate);
    ...
    var roundTo = 1000;
    var defaultRoundTo = 20;
    var lRoundTo = Math.min( roundTo, defaultRoundTo );
    ...
    CRData.forEach(function(CRDatum) {
        ...
        endTime = endArr[4];
        if( CRInitialCarbTime < dayStartTime && inputs.params["strict-24h-meals"]) return;
        if( CRInitialCarbTime > dayEndTime && inputs.params["strict-24h-meals"]) return;
        console.error(
        ...
        for (i=0; i < CSFGlucose.length; ++i) {
            ...
            if ( CSFGlucose[i].mealAbsorption === "start" ) {
                ...
                if(inputs.params["dbg-output"] & 4 ) console.error( CSFGlucose[i].mealAbsorption, " meal absorption ", CSFGlucose[i].dateString, " mealCarbs", mealCarbs );
                ...
                if(inputs.params["dbg-output"] & 4 ) console.error( CSFGlucose[i].mealAbsorption, " meal absorption ", CSFGlucose[i].dateString, " totalMealCarbs ",totalMealCarbs," totalDeviations ", totalDeviations);
            } else {
    ...
    for (var hour=0; hour < 24; hour++) {
        ...
        for (i=0; i < basalGlucose.length; ++i) {
            ...
            if (hour === myHour) {
                if( inputs.params["dbg-output"] & 8 ) {console.error('myHour ',myHour," BGTime ",BGTime," dev ", basalGlucose[i].deviation);}
        ...
        var basalNeeded = (1-autotuneMin) * 0.75 * deviations / ISF;
        ...
        if (basalNeeded > 0 ) {
            ...
            for (var offset=-5; offset <= 0; offset++) {
                ...
                var offsetHourPrev = offsetHour-1;
                if (offsetHourPrev < 0) { offsetHourPrev += 24; }
                //console.error(offsetHour);
                function apply_hour(hour,offset) {
                    var offsetHour = hour + offset;
                    if (offsetHour < 0) { offsetHour += 24; }
                    if( inputs.params["dbg-output"] & 8 ) console.error("Hour", offsetHour," is getting", actFractions[Math.abs(offset)] * .5," of total amount of ",basalNeeded,"adjustment");
                    savedRate =  newHourlyBasalProfile[offsetHour].rate;
                    newHourlyBasalProfile[offsetHour].rate += basalNeeded * actFractions[Math.abs(offset)] * .5;
                    newHourlyBasalProfile[offsetHour].rate=Math.round(newHourlyBasalProfile[offsetHour].rate*1000)/1000
                    if( !(offsetHour in hoursTuned) ) hoursTuned[offsetHour] = 1;
                    if( inputs.params["dbg-output"] & 8 ) console.error(hoursTuned);
                    if( "untuned" in  newHourlyBasalProfile[offsetHour]) delete newHourlyBasalProfile[offsetHour].untuned;
                    if( inputs.params["dbg-output"] & 8 ) console.error("Hour", offsetHour,"old rate", savedRate ," new rate",newHourlyBasalProfile[offsetHour].rate,newHourlyBasalProfile[offsetHour]);
                }
                if( inputs.params["dbg-output"] & 8 ) console.error("Hour", offsetHour," is getting", actFractions[Math.abs(offset)] * .5," of total amount of ",basalNeeded,"adjustment");
                if( inputs.params["dbg-output"] & 8 ) console.error("Hour", offsetHourPrev," is getting", actFractions[Math.abs(offset)] * .5," of total amount of ",basalNeeded,"adjustment");
                savedRate =  newHourlyBasalProfile[offsetHour].rate;
                newHourlyBasalProfile[offsetHour].rate += basalNeeded * actFractions[Math.abs(offset)] * .5;
                newHourlyBasalProfile[offsetHour].rate=Math.round(newHourlyBasalProfile[offsetHour].rate*1000)/1000
                if( !(offsetHour in hoursTuned) ) hoursTuned[offsetHour] = 1;
                ...
                if( "untuned" in  newHourlyBasalProfile[offsetHour]) delete newHourlyBasalProfile[offsetHour].untuned;
                if( inputs.params["dbg-output"] & 8 ) console.error("Hour", offsetHour,"old rate", savedRate ," new rate",newHourlyBasalProfile[offsetHour].rate,newHourlyBasalProfile[offsetHour]);
                savedRatePrev =  newHourlyBasalProfile[offsetHourPrev].rate;
                newHourlyBasalProfile[offsetHourPrev].rate += basalNeeded * actFractions[Math.abs(offset)] * .5;
                newHourlyBasalProfile[offsetHourPrev].rate=Math.round(newHourlyBasalProfile[offsetHourPrev].rate*1000)/1000
                if( !(offsetHourPrev in hoursTuned) ) hoursTuned[offsetHourPrev] = 1;
                if( inputs.params["dbg-output"] & 8 ) console.error(hoursTuned);
                if( "untuned" in  newHourlyBasalProfile[offsetHourPrev]) delete newHourlyBasalProfile[offsetHourPrev].untuned;
                if( inputs.params["dbg-output"] & 8 ) console.error("Hour", offsetHourPrev,"old rate", savedRatePrev ," new rate",newHourlyBasalProfile[offsetHourPrev].rate,newHourlyBasalProfile[offsetHour]);
            }
        ...
        // and adjust all of them downward proportionally
            ...
            for (offset=-5; offset <= 0; offset++) {
                ...
                sixHourBasal += newHourlyBasalProfile[offsetHour].rate;
            }
            var flattenedRatio = basalNeeded / sixHourBasal;
            if ( flattenedRatio  < -1 ) {
                console.error("oops, we don't have ",basalNeeded,", so limiting ratio to profile.autosens_min",autotuneMin," new needed", sixHourBasal*autotuneMin);
                basalNeeded = -sixHourBasal*autotuneMin;
            }
            //console.error("basal needed", basalNeeded,"sixHourBasal",sixHourBasal, "adjustmentRatio",adjustmentRatio);
            for (offset=-5; offset <= 0; offset++) {
                offsetHour = hour + offset;
                if (offsetHour < 0) { offsetHour += 24; }
                var offsetHourPrev = offsetHour-1;
              ...
              //  var adjustmentRatio = 1.0 ;
                if( inputs.params["dbg-output"] & 8 ) console.error("Hour", offsetHour," is getting", actFractions[Math.abs(offset)] * .5," of total amount of ",basalNeeded,"adjustment");
                if( inputs.params["dbg-output"] & 8 ) console.error("Hour", offsetHourPrev," is getting", actFractions[Math.abs(offset)] * .5," of total amount of ",basalNeeded,"adjustment");
                var newHourlyBasal = (newHourlyBasalProfile[offsetHour].rate -Math.abs(basalNeeded) * actFractions[ Math.abs(offset)]* .5);
                if( newHourlyBasal <= 0 ) {
                    if ( roundTo )
                        newHourlyBasal = 1/roundTo;
                    else
                        newHourlyBasal = 1/1000;
                }

                savedRate =  newHourlyBasalProfile[offsetHour].rate;
                newHourlyBasalProfile[offsetHour].rate = newHourlyBasal;
                newHourlyBasalProfile[offsetHour].rate=Math.round(newHourlyBasalProfile[offsetHour].rate*1000)/1000;
                if( inputs.params["dbg-output"] & 8 ) console.error("Hour", offsetHour,"old rate", savedRate ," new rate",newHourlyBasalProfile[offsetHour].rate);
                newHourlyBasal = (newHourlyBasalProfile[offsetHourPrev].rate -Math.abs(basalNeeded) * actFractions[ Math.abs(offset)]* .5);
                if( newHourlyBasal <= 0 ) {
                    if ( roundTo )
                        newHourlyBasal = 1/roundTo;
                    else
                ...
                savedRate =  newHourlyBasalProfile[offsetHourPrev].rate;
                newHourlyBasalProfile[offsetHourPrev].rate = newHourlyBasal;
                newHourlyBasalProfile[offsetHourPrev].rate=Math.round(newHourlyBasalProfile[offsetHourPrev].rate*1000)/1000;
                if( inputs.params["dbg-output"] & 8 ) console.error("Hour", offsetHourPrev,"old rate", savedRate ," new rate",newHourlyBasalProfile[offsetHourPrev].rate);
            }
        }
    }
    var lastAdjustedHour = -1;;
    var firstAdjustedHour = 24;
    var adjustedHours = [];
    if (pumpBasalProfile && pumpBasalProfile[0]) {
        for (hour=0; hour < 24; hour++) {
            //console.error("Top limit for hour",hour," clean ", newHourlyBasalProfile[hour]*autotuneMax, " min incr ",
            //    hourlyPumpProfile[hour].rate+1/roundTo," current rate ",  hourlyPumpProfile[hour].rate, " pump basal incr",1/roundTo);
            ...
            if( Math.round(newHourlyBasalProfile[hour].rate*lRoundTo)/lRoundTo != Math.round(hourlyPumpProfile[hour].rate*lRoundTo)/lRoundTo  ||  hour in hoursTuned) {
                if( hour < firstAdjustedHour ) {
                        firstAdjustedHour = hour;
                }
                if( hour > lastAdjustedHour ) {
    ...
    if( firstAdjustedHour == 24 ) {
        firstAdjustedHour = 0;
    }
    if( lastAdjustedHour == -1 ) {
        lastAdjustedHour = 23;
    }
    console.error("firstAdjustedHour ",firstAdjustedHour," lastAdjustedHour ", lastAdjustedHour, " adjustedHours ", adjustedHours);
    ...
    var oldTotalDailyBasal = 0;
    var totalDailyBasal = 0;
    for (hour=0; hour < 24; hour++) {
        //if (Math.round(hourlyBasalProfile[hour].rate*roundTo)/roundTo === Math.rouAnd(newHourlyBasalProfile[hour].rate*roundTo)/roundTo) {
        if (adjustedHours.indexOf(hour)  == -1) {
            var nextAdjustedHour = lastAdjustedHour;
            for (var nextHour = hour; nextHour < 24; nextHour++) {
		//console.error(" before lRoundTo=",lRoundTo," nh ",nextHour, hourlyBasalProfile[nextHour].rate, newHourlyBasalProfile[nextHour].rate);
                if (! (Math.round(hourlyBasalProfile[nextHour].rate*lRoundTo)/lRoundTo === Math.round(newHourlyBasalProfile[nextHour].rate*lRoundTo)/lRoundTo) ) {
	    ...
	    if( hour >= nextAdjustedHour ) {
                nextAdjustedHour = firstAdjustedHour;
            }
    ...
    for (hour=0; hour < 24; hour++) {
        var tmpRate =  Math.round( newHourlyBasalProfile[hour].rate*roundTo )/roundTo;
        var deltaRate = Math.round( (Math.max(tmpRate,hourlyBasalProfile[hour].rate) - Math.min(tmpRate,hourlyBasalProfile[hour].rate))*roundTo)/roundTo;
        if( (tmpRate/hourlyBasalProfile[hour].rate > autotuneMax ) && (deltaRate > 1/roundTo )) {
            console.error("Hour "+hour+" rate "+newHourlyBasalProfile[hour].rate+" rounded "+tmpRate+" (delta "+deltaRate+") is still too high");
            tmpRate -= 1/roundTo;
            console.error("further limiting to "+tmpRate);
        }
        if(tmpRate/hourlyBasalProfile[hour].rate < autotuneMin && deltaRate > 1/roundTo) {
            console.error("Hour "+hour+" rate "+newHourlyBasalProfile[hour].rate+" rounded "+tmpRate+" is still too low");
            tmpRate += 1/roundTo;
            console.error("further limiting to "+tmpRate);
        }
        newHourlyBasalProfile[hour].rate = tmpRate;
        oldTotalDailyBasal += hourlyPumpProfile[hour].rate;
        totalDailyBasal += newHourlyBasalProfile[hour].rate;
    }

    oldTotalDailyBasal = Math.round(oldTotalDailyBasal*roundTo)/roundTo;
    ...
    if ( scaledCR/totalCR < 0.75 || scaledCR/totalCR > saferScaleFactor ) console.error("WARNING: there's too big difference between scaledCR and totalCR");
    //if(inputs.params["dbg-output"] & 4 ) console.error("scaledCR (after 75% rule):", scaledCR);
    if( inputs.params["wizard-percent"] ) {
        fullNewCR = inputs.params["wizard-percent"]/100*fullNewCR;
    }
    if(inputs.params["dbg-output"] & 4 ) console.error("fullNewCR before min():", fullNewCR);
    fullNewCR = Math.min( fullNewCR, scaledCR );
    if(inputs.params["dbg-output"] & 4 ) console.error("fullNewCR after min():", fullNewCR);
    var weightFactor = .20;
    ...
    console.error("oldTotalDailyBasal =",oldTotalDailyBasal);
